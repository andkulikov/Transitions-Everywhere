...
public abstract class Visibility extends Transition {
    ...
    public Animator onDisappear(final ViewGroup sceneRoot,
        ...
        if (overlayView != null) {
            ...
            ViewGroupOverlayCompat.addOverlay(sceneRoot, overlayView, screenLoc[0], screenLoc[1]);
            Animator animator = onDisappear(sceneRoot, overlayView, startValues, endValues);
            if (animator == null) {
                ViewGroupOverlayCompat.removeOverlay(sceneRoot, overlayView);
            } else {
                final View finalOverlayView = overlayView;
                animator.addListener(new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        ViewGroupOverlayCompat.removeOverlay(sceneRoot, finalOverlayView);
                    }
                });
            }
            return animator;
            ...
            if (animator != null) {
                ...
                animator.addListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        if (!mCanceled) {
                            if (isForcedVisibility) {
                                ViewCompat.setTransitionAlpha(finalViewToKeep, 0);
                            } else {
                                finalViewToKeep.setVisibility(finalVisibility);
                            }
                        }
